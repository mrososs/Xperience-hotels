...
useSeoMeta({
  ...
  description: () => t('seo.meetings.description'),
  ogTitle: () => t('seo.meetings.title'),
  ogDescription: () => t('seo.meetings.description'),
  ogType: 'website',
  ...
  <section class="h-hero h-hero--page">
    ...
    <div class="h-hero__inner x-wrap">
      <div class="m-crumb x-reveal">
        ...
        <NuxtLink :to="localePath(`/resorts/${parkland?.slug ?? 'kiroseiz-parkland'}`)">{{ hotelName }}</NuxtLink>
        <i data-lucide="chevron-right"></i>
