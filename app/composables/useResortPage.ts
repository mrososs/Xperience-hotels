...
/** The slice of vue-i18n this module needs — passed in by the calling
 *  component (from its useLocale()), since the global instance is owned
 *  by @nuxtjs/i18n and only resolvable during component setup. */
export interface ResortPageI18n {
  t: (key: string) => string
  locale: Ref<string>
}
...
function initBooking(cleanups: Cleanup[], refreshers: Cleanup[], i18n: ResortPageI18n): OpenModal {
  const curLang = (): Lang => (i18n.locale.value as Lang) || 'en'
  const tr = (k: 'selectHotel' | 'addDates' | 'rooms' | 'guests' | 'nights'): string =>
    i18n.t('booking.' + k)
...
export async function setupResortPage(i18n: ResortPageI18n): Promise<ResortPageHandle> {
  const cleanups: Cleanup[] = []
  const refreshers: Cleanup[] = []
  initReveal(cleanups)
  initCounters(cleanups)
  const openModal = initBooking(cleanups, refreshers, i18n)
