...
export function useScrollSpy(
  ...
  // destination page has rendered its sections. The immediate run also
  // fires during SSR, where there is no frame (or DOM) to measure.
  watch(
    () => toValue(enabled),
    () => {
      if (import.meta.client) requestAnimationFrame(measure)
    },
    { immediate: true },
  )
